...
<div class="container mx-auto px-4 py-8">
                    ...
                    <div class="mt-4 flex items-center">
                        <form action="{{ route('cart.update', $item->id) }}" method="POST" class="cart-form">
                            ...
                            <div class="flex items-center">
                                ...
                                <input type="submit" value="Update" class="btn btn-secondary ml-2">
                            </div>
                        </form>
                        ...
                        <span class="ml-auto font-bold mx-2">${{ $item->price * $item->quantity }}</span>
                    </div>
                </div>
            </div>
        @endforeach
    ...
    <h2 class="text-lg font-bold mb-2 mt-4">Total: {{ $total }}</h2>

    <form action="{{ route('confirm') }}" method="POST" class="mt-4">
        @csrf
        <div class="mb-4">
            <label for="address" class="block text-sm font-medium text-gray-700">Address</label>
            <input type="text" name="address" id="address" value="{{ $user->address }}" class="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm">
        </div>
